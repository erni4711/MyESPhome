...
function t(key) {
  ...
  async function saveNormalTileBorders(enabled) {
    ...
    try {
      const response = await fetch('/admin/display/tile-borders', {
        method: 'POST',
  ...
  async function createScreenshotAndDownload() {
    ...
    try {
      const res = await fetch('/admin/screenshot', { method: 'POST' });
      ...
      link.href = '/admin/screenshot/download?ts=' + Date.now();
  ...
  async function downloadCrashLog() {
    try {
      const res = await fetch('/admin/crashlog?ts=' + Date.now());
      if (res.status === 404) {
  ...
  async function eraseCoreDump() {
    ...
    try {
      const res = await fetch('/admin/coredump/erase', { method: 'POST' });
      const data = await res.json().catch(() => ({}));
      if (!res.ok || !data.success) {
  ...
  async function loadFileManager(path = null) {
    ...
    try {
      const res = await fetch(fileManagerUrl('/admin/files/list', {
  ...
  function downloadFileManagerFile(path) {
    window.location.href = fileManagerUrl('/admin/files/download', {
      fs: fileManagerState.fs,
      path: path
    });
  }
  ...
  async function createFileManagerFolder() {
    ...
    if (!trimmed) return;
    try {
      await postFileManagerForm('/admin/files/mkdir', {
        fs: fileManagerState.fs,
        path: fileManagerState.path || '/',
        name: trimmed
      });
  ...
  async function renameFileManagerEntry(path, currentName) {
    ...
    try {
      await postFileManagerForm('/admin/files/rename', {
        fs: fileManagerState.fs,
        path,
        name: trimmed
      });
  ...
  async function deleteFileManagerEntry(path, name, isDir) {
    ...
    try {
      await postFileManagerForm('/admin/files/delete', {
        fs: fileManagerState.fs,
        path
      });
      invalidateScreensaverEditor();
      ...
      for (let fileIndex = 0; fileIndex < files.length; fileIndex++) {
        ...
        while (offset < file.size || firstPart) {
          ...
          setFileManagerStatus(
            ...
            ': ' + file.name + ' - ' + pct + '%');
          const res = await fetch(fileManagerUrl('/admin/files/upload', {
            fs: fileManagerState.fs,
            path: fileManagerState.path || '/',
  ...
  function waitForGithubFirmwareResult(targetTag) {
    ...
    const poll = async () => {
      try {
        const res = await fetch('/admin/ota/github/status?ts=' + Date.now(), {
          method: 'GET',
          cache: 'no-store',
  ...
  async function installGithubFirmware(tag) {
    ...
    try {
      const res = await fetch('/admin/ota/github/install', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
  ...
  async function checkOrInstallGithubFirmware() {
    ...
    try {
      const res = await fetch('/admin/ota/github/check', {
        method: 'POST',
        cache: 'no-store',
        credentials: 'same-origin'
      });
  ...
  async function uploadOtaFirmware() {
    ...
    try {
      ...
      const prepRes = await fetch('/admin/ota/prepare?size=' + otaSize + '&filename=' + otaFilename, {
        method: 'POST',
        cache: 'no-store',
        credentials: 'same-origin'
      });
    ...
    const otaSize = encodeURIComponent(String(file.size || 0));
    const otaFilename = encodeURIComponent(String(file.name || ''));
    xhr.open('POST', '/admin/ota/upload/raw?size=' + otaSize + '&filename=' + otaFilename, true);
  ...
  function fetchSensorMetaCache(force = false) {
    ...
    sensorMetaFetchInFlight = fetch('/admin/sensor_values')
      .then(res => res.json())
      .then(raw => {
  ...
  function fetchEntityOptions(force = false) {
    ...
    entityOptionsFetchInFlight = fetch('/admin/entity_options')
      .then(res => {
  ...
  let navigateFolderOptions = null;
  ...
  function ensureNavigateTargetOption(folderId, label) {
    ...
    const existing = navigateFolderOptions.find(option => option.value === value);
    if (existing) {
      existing.label = label;
    } else {
      navigateFolderOptions.push({value, label});
    }
  ...
  function syncFolderFragmentWithRoot(tabEl) {
    ...
    const folderOptions = navigateFolderOptions || Array.from(
  ...
  async function ensureFolderTabUi(folderId, name = null, icon = null) {
    ...
    folderTabLoadPromises[folderNum] = (async () => {
      const res = await fetch(
        '/admin/folders/tab?folder_id=' + encodeURIComponent(folderNum));
      const data = await res.json().catch(() => ({}));
  ...
  function loadTileData(index, tab) {
    ...
    fetch('/admin/tiles?folder=' + encodeURIComponent(folderId) + '&index=' + index)
      .then(res => res.json())
  ...
  function deleteFolder(tab) {
    ...
    fetch('/admin/folders/delete', { method: 'POST', body: formData })
      .then(res => res.json())
      .then(data => {
        if (data.success) {
  ...
  async function exportTilesConfig() {
    try {
      const foldersRequest = fetch('/admin/folders').then(async res => {
      ...
      const screensaverConfigRequest = fetch('/admin/screensaver').then(async res => {
        const data = await res.json();
        if (!res.ok || !data?.success) throw new Error('Screensaver config export failed');
        return data;
      });
      const screensaverGridRequest = fetch(
        '/admin/tiles?folder=' + encodeURIComponent(SCREENSAVER_FOLDER_ID)
      ).then(async res => {
        const data = await res.json();
      ...
      const tilesLists = await Promise.all(folders.map(async folder => {
        const response = await fetch(
          '/admin/tiles?folder=' + encodeURIComponent(folder.id));
        const data = await response.json();
  ...
  async function fetchTilesForImport(folderId) {
    const res = await fetch('/admin/tiles?folder=' + encodeURIComponent(folderId));
  ...
  function fetchTileGridData(tab, force = false) {
    ...
    tileDataLoadPromises[tab] = fetch(
      '/admin/tiles?folder=' + encodeURIComponent(folderId))
      .then(async response => {
  ...
  function initScreensaverEditor() {
    ...
    fetch('/admin/screensaver').then(r => r.json()).then(config => {
  ...
  function renderScreensaverEditor() {
    ...
    if (d.use_wallpapers && wallpaper && wallpaper.file_name) {
      const wanted = '/admin/screensaver/wallpaper?name=' + encodeURIComponent(wallpaper.file_name);
  ...
  async function saveHardwareIoNow() {
    ...
    let saved = false;
    try {
      const response = await fetch('/admin/hardware-io', {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    loadTileClipboard();
    loadNavigateTargetFolders().catch(error => {
      console.error('Navigate target folder load failed:', error);
    });
...
function normalizeIconName(value) {
  ...
  async function applyFolderPin(tab) {
    ...
    try {
      ...
      const response = await fetch('/admin/folders/access', {
        method: 'POST',
        headers: {'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8'},
        body
      });
