...
AUTO_LOAD = ["web_server_base", "web_server", "http_request", "spiffs"]
...
async def to_code(config):
    ...
    try:
        ...
        fonts_src = comp_dir / "fonts"
        if fonts_src.is_dir():
            shutil.copytree(
                fonts_src, component_build_dir / "fonts", dirs_exist_ok=True
            )
        root_fonts_header = comp_dir / "ui_fonts.h"
        if root_fonts_header.exists():
            shutil.copy2(root_fonts_header, component_build_dir / "ui_fonts.h")
    except Exception as e:
