...
// Normalize legacy type-specific entity fields to the single public
// `entity_id` field. Legacy names are accepted only as import input.
static void normalize_tile_entity(JsonObject tile, bool prefer_legacy = false) {
  const char* field = "sensor_entity";
  switch (tile["type"] | 0) {
    case 5:  field = "switch_entity"; break;
    case 12: field = "weather_entity"; break;
    case 14: field = "energy_entity"; break;
    case 15: field = "media_entity"; break;
  ...
  if (prefer_legacy && tile[field].is<const char*>()) {
    tile["entity_id"] = tile[field].as<const char*>();
  } else if (!tile["entity_id"].is<const char*>()) {
    if (tile[field].is<const char*>()) {
      tile["entity_id"] = tile[field].as<const char*>();
    } else {
    tile["entity_id"] = "";
    }
...
static void remove_legacy_entity_fields(JsonObject tile) {
...
static std::vector<FolderMeta> readFolderMetaList() {
  ...
  if (f) {
    ...
    if (sz > 0 && sz < 8192) {
      ...
      if (fread(&raw[0], 1, static_cast<size_t>(sz), f) ==
...
static std::string buildFolderButtonHtml(const FolderMeta& m) {
  ...
  if (icon.rfind("mdi:", 0) == 0)
    icon = icon.substr(4);
  else if (icon.rfind("mdi-", 0) == 0)
    icon = icon.substr(4);
  ...
  if (!icon.empty()) {
    html += "<i class=\"mdi mdi-" + htmlEscape(icon) +
            "\" style=\"font-size:24px;\"></i>";
  }
  html += "<span style=\"font-size:14px;font-weight:600;\">" +
...
static void addTypeFields(std::string& h, const std::string& tid,
  ...
  h += "<div id=\"" + tid + "_" + group + "_fields\" class=\"type-fields\">" +
       inner + "</div>";
}

// Helper: entity select (starts empty; admin.js populates it via
// /admin/entity_options).
static std::string entitySelect(const std::string& id) {
  return "<select id=\"" + id +
         "\"><option value=\"\">-- Select --</option></select>";
}

// Forward declaration (definition follows later, after
// GridTileData/readTileGrid).
...
static std::string buildFolderTabHtml(const FolderMeta& m) {
  ...
  // Title
  h += "<label>Title</label>";
  h += "<input type=\"text\" id=\"" + tid +
       "_tile_title\" placeholder=\"Auto\">";

  // Icon
  h += "<label>Icon <small style=\"color:#8a8a8a;\">(<a "
       "href=\"https://pictogrammers.com/library/mdi/\" target=\"_blank\" "
       "style=\"color:#4db6ac;\">Material Design</a>)</small></label>";
  h += "<input type=\"text\" id=\"" + tid +
       "_tile_icon\" placeholder=\"e.g. thermometer\">";
  ...
  // ── Sensor fields (type 1 = Sensor, type 9 = Gauge shares them) ──────────
  {
    std::string inner;
    inner +=
        "<label>Sensor Entity</label>" + entitySelect(tid + "_sensor_entity");
    inner += "<label>Unit Override</label><input type=\"text\" id=\"" + tid +
             "_sensor_unit\" placeholder=\"Auto\">";
    ...
    inner += "<select id=\"" + tid +
             "_sensor_display_mode\">"
             "<option value=\"0\">Value</option>"
             "<option value=\"1\">Gauge</option>"
             "<option value=\"2\">Bar</option>"
             "<option value=\"3\">Graph</option></select>";
    inner += "<div id=\"" + tid + "_sensor_gauge_fields\">";
    inner += "<label>Gauge Min</label><input type=\"number\" id=\"" + tid +
             "_sensor_gauge_min\" placeholder=\"0\">";
    inner += "<label>Gauge Max</label><input type=\"number\" id=\"" + tid +
             "_sensor_gauge_max\" placeholder=\"100\">";
    inner += "</div>";
    ...
    inner += "<input type=\"hidden\" id=\"" + tid +
             "_sensor_value_font\" value=\"0\">";
    inner += "<input type=\"hidden\" id=\"" + tid +
             "_sensor_popup_open_mode\" value=\"1\">";
  ...
  // ── Weather fields (type 12) ──────────────────────────────────────────────
  {
    std::string inner;
    inner +=
        "<label>Weather Entity</label>" + entitySelect(tid + "_weather_entity");
  ...
  // ── Scene fields (type 2) ─────────────────────────────────────────────────
  {
    std::string inner;
    inner +=
        "<label>Scene / Script</label>" + entitySelect(tid + "_scene_alias");
  ...
  // ── Navigate fields (type 4) ──────────────────────────────────────────────
  {
    ...
    inner +=
        ...
        tid + "_folder_pin_enabled\"> Require PIN</label>";
    inner += "<div class=\"folder-pin-control\">";
    inner += "<div class=\"password-field\">";
    inner += "<input type=\"password\" id=\"" + tid +
             "_folder_pin\" placeholder=\"PIN\">";
    inner +=
        "<button type=\"button\" class=\"password-toggle\""
        " data-label-show=\"Show\" data-label-hide=\"Hide\">Show</button>";
    inner += "</div>";
    inner += "<button type=\"button\" id=\"" + tid +
             "_folder_pin_apply\""
             " onclick=\"applyFolderPin('" +
             tid + "')\">Save PIN</button>";
    inner += "</div>";
  ...
  {
    std::string inner;
    inner += "<label>Switch / Light Entity</label>" +
             entitySelect(tid + "_switch_entity");
    ...
    inner += "<input type=\"hidden\" id=\"" + tid +
             "_switch_popup_open_mode\" value=\"1\">";
  ...
  // ── Clock fields (type 9) ─────────────────────────────────────────────────
  {
    ...
    inner +=
        ...
        "<input type=\"checkbox\" id=\"" +
        tid + "_clock_show_time\" checked> Show Time</label>";
    inner +=
        "<label class=\"inline-checkbox\">"
        "<input type=\"checkbox\" id=\"" +
        tid + "_clock_show_date\"> Show Date</label>";
    inner +=
        "<label class=\"inline-checkbox\">"
        "<input type=\"checkbox\" id=\"" +
  ...
  // ── Text fields (type 10) ─────────────────────────────────────────────────
  {
    ...
    inner +=
        "<label>Text</label>"
        "<input type=\"text\" id=\"" +
        tid + "_text_value\" placeholder=\"Enter text\">";
    inner += "<input type=\"hidden\" id=\"" + tid +
             "_text_value_font\" value=\"0\">";
  ...
  {
    std::string inner;
    inner += "<label>Media Player Entity</label>" +
             entitySelect(tid + "_media_entity");
  ...
  // ── Animation fields (type 16) ────────────────────────────────────────────
  {
    std::string inner;
    inner +=
        "<label>Animation File</label>" + entitySelect(tid + "_animation_file");
    inner +=
        "<label>Speed (FPS)</label>"
        "<input type=\"number\" id=\"" +
        tid +
        "_animation_fps\" min=\"1\" max=\"30\" value=\"10\">"
        "<span id=\"" +
        tid + "_animation_fps_val\">10</span>";
    inner +=
        "<label>Zoom %</label>"
        "<input type=\"number\" id=\"" +
        tid +
        "_animation_zoom\" min=\"25\" max=\"300\" value=\"100\">"
        "<span id=\"" +
        tid + "_animation_zoom_val\">100</span>";
    inner +=
        "<input type=\"hidden\" id=\"" + tid + "_animation_fit\" value=\"0\">";
  ...
  // ── Climate / AC fields (type 17) ─────────────────────────────────────────
  {
    std::string inner;
    inner +=
        "<label>Climate Entity</label>" + entitySelect(tid + "_climate_entity");
  ...
  // ── Camera fields (type 18) ───────────────────────────────────────────────
  {
    std::string inner;
    inner +=
        "<label>Camera Entity</label>" + entitySelect(tid + "_camera_entity");
  ...
  h += "<button type=\"button\" class=\"btn\" onclick=\"copyTile('" + tid +
       "')\">Copy</button>";
  h += "<button type=\"button\" class=\"btn\" onclick=\"pasteTile('" + tid +
       "')\">Paste</button>";
  h +=
      "<button type=\"button\" class=\"btn btn-danger\" onclick=\"resetTile('" +
      tid + "')\">Delete</button>";
  h += "</div>";
...
static std::string buildTileGridHtml(const std::string& tid, int folder_id) {
  ...
  for (int i = 0; i < (int)tiles.size(); i++) {
    ...
    // Background colour
    std::string style;
    style += "grid-column:" + std::to_string(t.col + 1) + "/span " +
             std::to_string(t.span_w) + ";";
    style += "grid-row:" + std::to_string(t.row + 1) + "/span " +
             std::to_string(t.span_h) + ";";
    if (t.type != 0) {
      char hex[16];
    ...
    if (t.type == 4)  // Navigate tile
      h +=
          " data-navigate-target=\"" + std::to_string(t.navigate_target) + "\"";
    h += " draggable=\"true\"";
    h += " style=\"" + style + "\"";
    ...
    if (t.type != 0) {
      ...
      // Title
      if (!t.title.empty())
        h += "<div class=\"tile-title\" id=\"" + tid + "-tile-" +
             std::to_string(i) + "-title\">" + htmlEscape(t.title) + "</div>";
...
bool TilesHandler::canHandle(AsyncWebServerRequest* request) const {
  ...
  ESP_LOGD("web_admin_local.tiles",
           "canHandle TilesHandler: url=%s method=%d base=%s", url.c_str(),
           request->method(), base_.c_str());
  const std::string prefix = base_ + "/tiles";
  // Only serve the HTML page for bare GET requests without a 'folder' query
  // parameter. Requests with ?folder=N are handled by ApiTilesHandler as JSON
  // API calls.
  return request->method() == HTTP_GET &&
         (url == prefix || url == prefix + "/") && !request->hasParam("folder");
}
...
void TilesHandler::handleRequest(AsyncWebServerRequest* request) {
  ...
  static const char kGlobals[] =
      ...
      "load:'loadSensorFields',save:'saveSensorFields',reset:'"
      "resetSensorFields'},"
      // 2 = scene / script
      ...
      "load:'loadNavigateFields',save:'saveNavigateFields',reset:'"
      "resetNavigateFields'},"
      ...
      "load:'loadWeatherFields',save:'saveWeatherFields',reset:'"
      "resetWeatherFields'},"
      // 14 = energy / consumption
      "'14':{label:'Energy',fields:'energy',"
      "load:'loadEnergyFields',save:'saveEnergyFields',reset:'"
      "resetEnergyFields'},"
      ...
      "load:'loadAnimationFields',save:'saveAnimationFields',reset:'"
      "resetAnimationFields'},"
      // 17 = climate / AC thermostat
      "'17':{label:'Climate',fields:'climate',"
      "load:'loadClimateFields',save:'saveClimateFields',reset:'"
      "resetClimateFields'},"
      // 18 = IP camera snapshot
      "'18':{label:'Camera',fields:'camera',"
      "load:'loadCameraFields',save:'saveCameraFields',reset:'"
      "resetCameraFields'},"
  ...
  std::string html =
      ...
      // Grid preview dimensions (7×5 layout for the standard Waveshare P4
      // board). Cell height: (430 - 2*12 - 10*(5-1)) / 5 = 73px; width scaled
      // proportionally.
      "<style>:root{"
      ...
      // Hidden settings slot used for the special network-settings tile (type
      // 7)
      +
      "<div id=\"settingsHiddenSlot\" class=\"settings-hidden-slot\" "
      "draggable=\"false\">"
      "<div id=\"settingsHiddenTile\" class=\"tile hidden-settings-tile\" "
      "draggable=\"true\">"
      "<span id=\"settingsHiddenHint\"></span>"
      "</div>"
      "</div>" +
      "</div></div>" + kGlobals + "<script src=\"" + adminJs +
      "\"></script>"
      "<script>"
      ...
      "window.switchTab=typeof "
      "switchTab==='function'?switchTab:window.switchTab;"
      "window.selectTile=typeof "
      "selectTile==='function'?selectTile:window.selectTile;"
      "window.openPreviewNavigation=typeof "
      "openPreviewNavigation==='function'?openPreviewNavigation:window."
      "openPreviewNavigation;"
      "window.updateTileType=typeof "
      "updateTileType==='function'?updateTileType:window.updateTileType;"
      "window.resetTileColor=typeof "
      "resetTileColor==='function'?resetTileColor:window.resetTileColor;"
      "window.copyTile=typeof copyTile==='function'?copyTile:window.copyTile;"
      "window.pasteTile=typeof "
      "pasteTile==='function'?pasteTile:window.pasteTile;"
      "window.resetTile=typeof "
      "resetTile==='function'?resetTile:window.resetTile;"
      "window.applyFolderPin=typeof "
      "applyFolderPin==='function'?applyFolderPin:window.applyFolderPin;"
      "window.openFolderSettings=typeof "
      "openFolderSettings==='function'?openFolderSettings:window."
      "openFolderSettings;"
      "window.saveFolderSettings=typeof "
      "saveFolderSettings==='function'?saveFolderSettings:window."
      "saveFolderSettings;"
      "window.closeFolderSettings=typeof "
      "closeFolderSettings==='function'?closeFolderSettings:window."
      "closeFolderSettings;"
      "if(window.ensureFolderSettingsButton)window.ensureFolderSettingsButton()"
      ";"
      // Standalone selectTile — works even when admin.js functions are not
      // global. If admin.js's selectTile WAS exported above, this is NOT
      // executed (guard below).
      ...
      "document.getElementById('FolderSettingsPanel')?.classList.add('hidden');"
      // Show settings panel
      "var sp=document.getElementById(tab+'Settings');"
      "if(sp){var "
      "ts=sp.querySelector('.tile-specific-settings');if(ts)ts.classList."
      "remove('hidden');}"
      ...
      "};"
      "}"
      // Standalone form population — populates settings panel from raw tile
      // data object.
      ...
      "var "
      "op=document.createElement('option');op.value=tv;op.textContent='Type "
      "'+tv;tsel.appendChild(op);}"
      "tsel.value=tv;"
      "}"
      // Title, icon
      ...
      "co.value='#'+('000000'+((data.bg_color||0)&0xFFFFFF).toString(16))."
      "slice(-6);}"
      // Layout
      ...
      "var emap=["
      "['sensor_entity',data.entity_id||data.sensor_entity],"
      "['switch_entity',data.switch_entity||data.entity_id||data.sensor_entity],"
      "['weather_entity',data.weather_entity||data.entity_id||data.sensor_entity],"
      "['energy_entity',data.energy_entity||data.entity_id||data.sensor_entity],"
      "['media_entity',data.media_entity||data.entity_id||data.sensor_entity],"
      ...
      "var "
      "opts=(window._entityOptions&&window._entityOptions[p[0].replace('_"
      "entity','')+'s'])||[];"
      "opts.forEach(function(opt){"
      "if(!Array.from(sel.options).some(function(o){return o.value===opt.v;})){"
      "var "
      "o=document.createElement('option');o.value=opt.v;o.textContent=opt.t||"
      "opt.v;"
      "sel.appendChild(o);}});"
      // Add current value if missing
      "if(!Array.from(sel.options).some(function(o){return o.value===p[1];})){"
      "var "
      "o=document.createElement('option');o.value=p[1];o.textContent=p[1];sel."
      "appendChild(o);}"
      "sel.value=p[1];"
      "});"
      "};"
      // Patch applyTileDataToEditor for configuredValue fix (if admin.js IS
      // accessible)
      "if(typeof applyTileDataToEditor==='function'){"
      "var _adt=applyTileDataToEditor;"
      "window.applyTileDataToEditor=applyTileDataToEditor=function(idx,tab,"
      "data){"
      "if(data&&typeof data==='object'&&!Array.isArray(data)&&data.entity_id&&"
      "!data.sensor_entity)data.sensor_entity=data.entity_id;"
      "_adt(idx,tab,data);"
      "if(!data||typeof data!=='object'||Array.isArray(data))return;"
      "["
      "[tab+'_sensor_entity',data.entity_id||data.sensor_entity],"
      "[tab+'_switch_entity',data.switch_entity||data.entity_id||data.sensor_entity],"
      "[tab+'_weather_entity',data.weather_entity||data.entity_id||data.sensor_entity],"
      "[tab+'_energy_entity',data.energy_entity||data.entity_id||data.sensor_entity],"
      "[tab+'_media_entity',data.media_entity||data.entity_id||data.sensor_entity],"
      ...
      "var "
      "el=document.getElementById(tab+'_tile_color');if(el)el.value='#2A2A2A';"
      "};"
      "}"
      // Standalone copyTile, pasteTile, resetTile (clipboard in localStorage)
      "if(typeof window.copyTile!=='function'){"
      "window.copyTile=function(tab){window._tileClipboard=window._"
      "collectTileForm(tab);console.log('copied');};"
      "}"
      "if(typeof window.pasteTile!=='function'){"
      "window.pasteTile=function(tab){if(window._tileClipboard)window._"
      "fillTileSettings(tab,window._tileClipboard);};"
      "}"
      "if(typeof window.resetTile!=='function'){"
      "window.resetTile=function(tab){"
      // Find currently selected tile index
      "var tileEl=document.querySelector('#tab-tiles-'+tab+' "
      ...
      "x-www-form-urlencoded;charset=UTF-8'},body:new URLSearchParams(fd)})"
      ".then(function(r){return r.json();})"
      ".then(function(d){"
      "if(d.success){"
      ...
      "var sp=document.getElementById(tab+'Settings');"
      "if(sp){var "
      "ts=sp.querySelector('.tile-specific-settings');if(ts)ts.classList.add('"
      "hidden');}"
      ...
      "window.applyFolderPin=function(tab){console.warn('applyFolderPin not "
      "implemented');};"
      "}"
      ...
      "'clock_show_time','clock_show_date','clock_time_format','clock_date_"
      "format',"
      ...
      "}"
      "var tileEl=document.querySelector('#tab-tiles-'+tabMatch+' "
      ".tile-grid>.tile.active');"
      ...
      "document.querySelectorAll('.tab-content').forEach(function(e){e."
      "classList.remove('active');});"
      "document.querySelectorAll('.tab-btn').forEach(function(e){e.classList."
      "remove('active');});"
      ...
      "};"
      // Ensure window.switchTab is ALWAYS callable — use admin.js version if
      // accessible, otherwise fall back to _swTab so onclick='switchTab(...)'
      // never throws.
      ...
      "});"
      "});"
      // Enable drag+resize on all tile tabs; safe to call again if admin.js
      // already did it (enableTileResize guards against double-binding;
      // enableTileDrag just adds extra listeners)
      "document.addEventListener('DOMContentLoaded',function(){"
      "setTimeout(function(){"
      ...
      "}"
      "if(typeof "
      "updateTileSettingsMaxHeight==='function')updateTileSettingsMaxHeight();"
...
bool ApiTilesHandler::canHandle(AsyncWebServerRequest* request) const {
  const auto url = request_url(request);
  ESP_LOGD("web_admin_local.api.tiles",
           "canHandle ApiTilesHandler: url=%s method=%d base=%s", url.c_str(),
           request->method(), base_.c_str());
  const std::string apiPrefix = std::string("/api") + "/tiles";
  const std::string adminPrefix = base_ + "/tiles";
  bool match = (url == apiPrefix || url == apiPrefix + "/" ||
                url == adminPrefix || url == adminPrefix + "/");
...
void ApiTilesHandler::handleRequest(AsyncWebServerRequest* request) {
  ESP_LOGI("web_admin_local.api.tiles",
  ...
  if (request->method() == HTTP_GET) {
    ...
    if (rf) {
      ...
      if (!deserializeJson(doc, raw)) {
        ...
        if (arr) {
          for (JsonObject tile : arr) {
            normalize_tile_entity(tile);
            remove_legacy_entity_fields(tile);
          }
        }
  ...
  if (request->method() == HTTP_POST) {
    ...
    // ─────────────────────────────── admin.js sends multipart/form-data with
    // fields: folder, index, col, row, span_w, span_h, type, title, icon_name,
    // bg_color / bg_color_default, and any type-specific entity / config
    // fields.
    if (request->hasArg("index")) {
      ESP_LOGD("web_admin_local.api.tiles", "FormData tile save, folder=%u",
               (unsigned)folder);
        ...
        if (rf) {
          ...
          if (sz > 0 && sz <= 65536) {
            ...
            if (!deserializeJson(rdoc, raw)) {
              ...
              else if (rdoc["tiles"].is<JsonArray>())
                rarr = rdoc["tiles"].as<JsonArray>();
              if (rarr) {
                int ri = 0;
      ...
      static const char* numFields[] = {"sensor_decimals",
                                        "sensor_value_font",
                                        "sensor_display_mode",
                                        "sensor_gauge_min",
                                        "sensor_gauge_max",
                                        "sensor_gauge_arc",
                                        "sensor_gauge_size",
                                        "sensor_gauge_y_offset",
                                        "sensor_value_y_offset",
                                        "sensor_graph_height",
                                        "popup_open_mode",
                                        "switch_style",
                                        "animation_fps",
      ...
      for (int i = 0; numFields[i]; i++)
        setFloatField(numFields[i], numFields[i]);

      // Serialize updated tile back into the strings array.
      normalize_tile_entity(tile, true);
      remove_legacy_entity_fields(tile);
      ...
      // Build response — include navigate_target so the SPA can update folder
      // links.
    ...
    if (doc["tiles"].is<JsonArray>()) {
      ...
      // are available to TilesHandler and ApiFolderHandler.
      ...
      char folder_key[16];
      snprintf(folder_key, sizeof(folder_key), "%u",
               static_cast<unsigned>(folder));
      if (doc["grids"][folder_key].is<JsonArray>()) {
        ...
        if (doc["grids"]["0"].is<JsonArray>()) {
          ...
          ESP_LOGW("web_admin_local.api.tiles", "No grid found for folder %u",
                   static_cast<unsigned>(folder));
    ...
    JsonArray normalized_tiles = normalized_doc.as<JsonArray>();
    if (!normalized_tiles) {
      request->send(400, "text/plain", "Tile payload is not an array");
      return;
    }
    for (JsonObject tile : normalized_tiles) {
      normalize_tile_entity(tile);
      remove_legacy_entity_fields(tile);
    }
    body_to_save.clear();
    serializeJson(normalized_tiles, body_to_save);
    ...
    if (written != body_to_save.size()) {
      ESP_LOGW("web_admin_local.api.tiles",
               "Incomplete write to tmp file: wrote %u expected %u",
               (unsigned)written, (unsigned)body_to_save.size());
...
void ApiTilesHandler::handleBody(AsyncWebServerRequest* request, uint8_t* data,
  ...
  if (!body_too_large_) {
    if (body_buf_.size() >
        kMaxTileRequestBody - std::min(len, kMaxTileRequestBody))
      body_too_large_ = true;
    else
      body_buf_.append(reinterpret_cast<const char*>(data), len);
  }
  ESP_LOGD("web_admin_local.api.tiles",
           "handleBody: index=%u len=%u total=%u accumulated=%u",
           (unsigned)index, (unsigned)len, (unsigned)total,
           (unsigned)body_buf_.size());
}

// ── ApiFolderHandler ─────────────────────────────────────────────────────────

ApiFolderHandler::ApiFolderHandler(const std::string& base) : base_(base) {}

bool ApiFolderHandler::canHandle(AsyncWebServerRequest* request) const {
  ...
  return url == adminBase || url == adminBase + "/" ||
         ...
         url == adminBase + "/settings" || url == apiBase ||
         url == apiBase + "/" || url == apiBase + "/tab" ||
         url == apiBase + "/tab/" || url == apiBase + "/settings";
}
...
std::string ApiFolderHandler::buildFolderSettingsHtml() {
  return "<div id=\"FolderSettingsPanel\" class=\"tile-settings "
         "folder-settings-panel\">"
         "<div class=\"tile-specific-settings\">"
         "<div class=\"tile-settings-head\"><h3 style=\"margin-top:0;\">Folder "
         "Settings</h3></div>"
         "<div class=\"tile-settings-body\">"
         "<label for=\"folder_settings_name\">Folder name</label>"
         "<input type=\"text\" id=\"folder_settings_name\">"
         "<label for=\"folder_settings_icon\">Icon <small "
         "style=\"color:#8a8a8a;\">(<a href=\"https://pictogrammers.com/library/mdi/\" target=\"_blank\" style=\"color:#4db6ac;\">Material Design</a>)</small></label>"
         "<input type=\"text\" id=\"folder_settings_icon\" placeholder=\"e.g. "
         "folder\">"
         "<div style=\"display:flex;gap:8px;margin-top:14px;\">"
         "<button type=\"button\" class=\"btn\" "
         "onclick=\"saveFolderSettings()\">Save</button>"
         "<button type=\"button\" class=\"btn\" "
         "onclick=\"createFolder()\">Create Folder</button>"
         "<button type=\"button\" class=\"btn transparent\" "
         "onclick=\"closeFolderSettings()\">Cancel</button>"
         "</div>"
         "</div>"
         "</div>"
         "</div>";
...
void ApiFolderHandler::handleRequest(AsyncWebServerRequest* request) {
  const auto url = request_url(request);
  const bool is_tab = url.find("/tab") != std::string::npos;
  if (url.find("/settings") != std::string::npos) {
    request->send(200, "text/html; charset=utf-8",
                  buildFolderSettingsHtml().c_str());
    return;
  }

  // GET /admin/folders or /api/folders — folder list from files on disk.
  // Format:
  // {"success":true,"folders":[{"id":0,"parent_id":0,"name":"Home","icon_name":""},...]}
  if (!is_tab) {
    if (request->method() == HTTP_POST) {
      if (request->hasArg("action") && request->arg("action") == "create") {
        auto metas = readFolderMetaList();
        int folder_id = 0;
        for (const auto& m : metas) {
          if (m.id >= folder_id) folder_id = m.id + 1;
        }
        if (folder_id > 9) {
          request->send(
              400, "application/json; charset=utf-8",
              "{\"success\":false,\"error\":\"Folder limit reached\"}");
          return;
        }
        FolderMeta created;
        created.id = folder_id;
        created.parent_id = 0;
        created.name = request->hasArg("name")
                           ? request->arg("name")
                           : ("Folder " + std::to_string(folder_id + 1));
        created.icon =
        ...
        if (!f) {
          request->send(
              ...
              "{\"success\":false,\"error\":\"Unable to save folders\"}");
          return;
        }
        std::string json = "{\"folders\":[";
        ...
        const std::string response =
            "{\"success\":true,\"folder_id\":" + std::to_string(created.id) +
            "}";
        request->send(200, "application/json; charset=utf-8", response.c_str());
        return;
      }
      if (!request->hasArg("folder_id") || !request->hasArg("name") ||
          !request->hasArg("icon_name")) {
        request->send(
            400, "application/json; charset=utf-8",
            "{\"success\":false,\"error\":\"Missing folder fields\"}");
        return;
      }
      const int folder_id = atoi(request->arg("folder_id").c_str());
      auto metas = readFolderMetaList();
      bool found = false;
      for (auto& m : metas) {
        if (m.id != folder_id) continue;
        m.name = request->arg("name");
        m.icon = request->arg("icon_name");
        found = true;
        break;
      }
      if (!found) {
  ...
  // GET /admin/folders/tab or /api/folders/tab?folder_id=N — tab+button HTML
  // for dynamic loading.
  if (is_tab) {
    int folder_id = 0;
