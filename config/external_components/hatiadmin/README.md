...
### Fetching states with the tools script

The WebSocket `get_states` response can be saved to a JSON file for
troubleshooting or offline inspection. Install the one host-side dependency
and run the script from its directory:

```powershell
python -m pip install websocket-client
python tools\get-ha-states.py `
  --url http://homeassistant.local:8123 `
  --output states.json
```

By default, the script reads `config/secrets.yaml` and looks for
`ha_long_lived_access_token` (also accepting `HA_TOKEN` and the other token
names used by this project). Use `--secrets path\to\secrets.yaml` to select a
different file, or `--token` to explicitly override the file. The token is
sent only during WebSocket authentication and is not written to the output
file or printed by the script. The output file contains the state array
returned by Home Assistant.

### Backing up and restoring a tile folder

Use `tools\tile-folder.py` to download one folder from the device or upload it
back later. The script uses only Python's standard library:

```powershell
python tools\tile-folder.py download `
  --url http://192.168.10.26/admin/tiles `
  --folder 1 `
  --file backups\folder-1.json

python tools\tile-folder.py upload `
  --url http://192.168.10.26/admin/tiles `
  --folder 1 `
  --file backups\folder-1.json
```

Use `--folder all` to back up or restore all ten folders. The backup uses a
`{"grids":{"0":[...],"1":[...]}}` JSON object and each folder is transferred
individually to the device:

```powershell
python tools\tile-folder.py download `
  --url http://192.168.10.26/admin/tiles `
  --folder all `
  --file backups\tiles-all.json
...
Uploads are validated as JSON tile arrays before they are sent. The script
sends each folder in the `{"tiles":[...]}` format expected by HATiAdmin. Folder
numbers are limited to `0` through `9` (or `all`), and each device request is
capped at 128 KiB to match the firmware API.
