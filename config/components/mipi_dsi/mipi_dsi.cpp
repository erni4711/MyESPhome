...
static inline uint16_t swap_rb_565(uint16_t pixel) {
  return static_cast<uint16_t>(((pixel & 0x001F) << 11) | (pixel & 0x07E0) | ((pixel & 0xF800) >> 11));
}
...
void MIPI_DSI::draw_pixels_at(int x_start, int y_start, int w, int h, const uint8_t *ptr, display::ColorOrder order,
  ...
  bool need_color_swap = (bitness == display::COLOR_BITNESS_565 && this->color_depth_ == display::COLOR_BITNESS_565 &&
                          order != this->color_mode_);
  bool need_endian_swap = (bitness == display::COLOR_BITNESS_565 && this->color_depth_ == display::COLOR_BITNESS_565 &&
                           big_endian != this->byte_order_big_);

  if (need_color_swap || need_endian_swap) {
    size_t line_stride = x_offset + w + x_pad;
    std::vector<uint16_t> line(static_cast<size_t>(w));
    for (int y = 0; y != h; y++) {
      const uint8_t *src = ptr + ((y_offset + y) * line_stride + x_offset) * 2;
      for (int x = 0; x != w; x++) {
        uint16_t pixel = big_endian ? (static_cast<uint16_t>(src[0]) << 8) | src[1]
                                    : static_cast<uint16_t>(src[0]) | (static_cast<uint16_t>(src[1]) << 8);
        if (need_color_swap) {
          pixel = swap_rb_565(pixel);
        }
        if (this->byte_order_big_) {
          pixel = static_cast<uint16_t>((pixel << 8) | (pixel >> 8));
        }
        line[static_cast<size_t>(x)] = pixel;
        src += 2;
      }
      this->write_to_display_(x_start, y_start + y, w, 1, reinterpret_cast<const uint8_t *>(line.data()), 0, 0, 0);
    }
    return;
  }
...
void MIPI_DSI::draw_pixel_at(int x, int y, Color color) {
  ...
  switch (this->color_depth_) {
    case display::COLOR_BITNESS_565: {
      ...
      uint8_t hi_byte = static_cast<uint8_t>(r & 0xF8) | (color.g >> 5);
      uint8_t lo_byte = static_cast<uint8_t>((color.g & 0x1C) << 3) | (b >> 3);
      uint16_t new_color = this->byte_order_big_ ? static_cast<uint16_t>(hi_byte | (lo_byte << 8))
                                                 : static_cast<uint16_t>(lo_byte | (hi_byte << 8));
