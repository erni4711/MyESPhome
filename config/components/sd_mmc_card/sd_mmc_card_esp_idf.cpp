...
bool SdMmc::append_file_chunk(const std::string &path, const uint8_t *data, size_t len, bool create_if_missing) {
  const char *mode = create_if_missing ? "wb" : "ab";
  std::string absolute_path = build_path(path.c_str());
  FILE *file = fopen(absolute_path.c_str(), mode);
  if (file == nullptr) {
    ESP_LOGE(TAG, "Failed to open file for append: %s", absolute_path.c_str());
    return false;
  }
  size_t written = fwrite(data, 1, len, file);
  fclose(file);
  if (written != len) {
    ESP_LOGE(TAG, "Failed to append file chunk: %s", absolute_path.c_str());
    return false;
  }
  return true;
}

bool SdMmc::stream_file(const std::string &path, const std::function<bool(const uint8_t *, size_t)> &writer,
                        size_t chunk_size) {
  std::string absolute_path = build_path(path.c_str());
  FILE *file = fopen(absolute_path.c_str(), "rb");
  if (file == nullptr) {
    ESP_LOGE(TAG, "Failed to open file for streaming: %s", absolute_path.c_str());
    return false;
  }
  std::vector<uint8_t> buffer(chunk_size);
  while (true) {
    size_t read_len = fread(buffer.data(), 1, buffer.size(), file);
    if (read_len == 0) {
      break;
    }
    if (!writer(buffer.data(), read_len)) {
      fclose(file);
      return false;
    }
  }
...
bool SdMmc::stat_file(const std::string &path, size_t *size, bool *is_dir) const {
  std::string absolute_path = build_path(path.c_str());
  struct stat info;
...
std::string SdMmc::list_dir_json(const std::string &path) {
  auto entries = this->list_directory_file_info(path, 0);
  std::string json = "[";
  bool first = true;
  for (const auto &entry : entries) {
    ...
    first = false;
    json += "{\"name\":\"" + name + "\",\"size\":" + std::to_string(entry.size) +
